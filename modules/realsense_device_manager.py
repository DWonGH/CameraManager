...
class DeviceManager:
    ...
    def enable_specified_devices(self, specified_devices, enable_ir_emitter=False):
        """
        Enable all the Intel RealSense Devices which are connected to the PC
        """
        for serial in self._available_devices:
            if serial in specified_devices:
    ...
    def get_device_intrinsics(self, frames):
        ...
        for (serial, frameset) in frames.items():
            ...
            for key, value in frameset.items():
                device_intrinsics[serial][key] = str(value.get_profile().as_video_stream_profile().get_intrinsics())
